...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // LIRA: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "LIRA Signed Message:\n";
...
{
    // LIRA: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // LIRA: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // LIRA: 1 days
static const int64 nTargetSpacing = 120; // LIRA: 2 minute blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // LIRA: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // LIRA: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // LIRA: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "CNN January 26, 2014 Updated 0258 GMT Ukraine opposition leaders reject offer of government posts";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1300000000; //1390721511
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // LIRA: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LIRAMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LIRAMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LIRAMiner(CWallet *pwallet)
{
    printf("LIRAMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("LIRA-miner");
    ...
    try { loop {
        ...
        printf("Running LIRAMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LIRAMiner, pwallet));
}
